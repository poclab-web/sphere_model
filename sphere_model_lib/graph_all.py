...
for i, (nu, label) in enumerate(zip(["BH3", "LiAlH4", "NaBH4", "LiAl(OMe)3H", "MeLi", "MeMgI", "PhLi", "PhMgI"],
                                    ["{BH_3}", "{LiAlH_4}", "{NaBH_4}", "{LiAl(OMe)_3H}", "{MeLi}", "{MeMgI}", "{PhLi}",
                                     "{PhMgI}"])):
    ax = fig.add_subplot(2, 4, i + 1)
    for param_file_name, color, level in zip(glob.glob("../parameter/run_sphere_model_parameter_1120/*.txt"),
                                             [["red", "red"], ["blue", "blue"], ["green", "green"]],
                                             ["b3lyp/6-31g(d)//b3lyp/6-31g(d)", "M06-2X/6-31g(d)//b3lyp/6-31g(d)", "M06-2X/6-311g(d,p)//b3lyp/6-31g(d)"]):
...
for i, (name, label) in enumerate(zip(["LiAlH4", "NaBH4", "MeLi","PhLi"], ["{LiAlH_4}", "{NaBH_4}", "{MeLi}","{PhLi}"])):
...
for i,(param_file_name, color) in enumerate(zip(glob.glob("../parameter/run_sphere_model_parameter_1120/*.txt"),
    ...
    bar1=ax.bar(pos, l[:,1]/ np.sum(np.abs(l),axis=1),width= totoal_width/3, bottom=1, color="blue",alpha=0.2+0.2*np.sqrt(i))  # ,label="orbital"
    bar2=ax.bar(pos, l[:,0]/ np.sum(np.abs(l),axis=1), width= totoal_width/3,color="red",alpha=0.2+0.2*np.sqrt(i))  # ,label="steric"
...
if True:
    ...
    for j,(param_file_name, color) in enumerate(zip(glob.glob("../parameter/run_sphere_model_parameter_1120/*.txt"),
        ...
        l = []
        for i, (name, label) in enumerate(zip(["BH3", "LiAlH4", "NaBH4", "PhLi", "LiAl(OMe)3H", "MeLi", "MeMgI", "PhMgI"],
        ...
        if True:#_=="loo":
            label = "LOOCV " + "$q^2$" + " = {:.2f}\nRMSE = {:.2f} kcal/mol".format(r2_score(df_train["ΔΔG.expt."], df_train["ΔΔG.loo"]),
        ...
        l = []
        for i, (name, label) in enumerate(zip(["LiAlH4", "NaBH4", "MeLi","PhLi"], ["{LiAlH4}", "{NaBH_4}", "{MeLi}","{PhLi}"])):
            df_test = pd.read_excel("{}/{}/test_prediction.xls".format(param["save_dir"], name))
            if name=="PhLi":
        ...
        if True:#_=="test":
            label="test "+"${r_\mathrm{test}^2}$"+" = {:.2f}\nRMSE = {:.2f} kcal/mol".format(r2_score(df_test["ΔΔG.expt."], df_test["ΔΔG.predict"]),np.sqrt(mean_squared_error(df_test["ΔΔG.predict"], df_test["ΔΔG.expt."])))
        else:
            label="test"
        ax.plot(df_test["ΔΔG.expt."], df_test["ΔΔG.predict"], "s", color="red",
                label=label,
                markersize=markersize, alpha=alpha)
        ax.text(-2.5, 2.0, "$\mathrm{N_{training}}$ = "+"{}\n".format(len(df_train))+"$\mathrm{N_{test}}$ = "+"{}".format(len(df_test)), fontsize=8)
        fig.tight_layout()  # レイアウトの設定
        ax.legend(fontsize=5, loc='lower right')  # 凡例
    ...
    plt.savefig("../figs/all/plot_all.png", dpi=300)